...
public:
	...
	void calc_grads( const tensor_t<double>& grad_next_layer ) {
		...
		//std::cout << "act_grad.size" << act_grad.size << std::endl;
		//std::cout << "activator_input.size" << activator_input.size << std::endl;
		//std::cout << "grad_next_layer.size" << grad_next_layer.size << std::endl;
                //{ int b = 1;
                        for ( int n = 0; n < activator_input.size.x; n++ ){
